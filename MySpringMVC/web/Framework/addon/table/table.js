...
(function (factory) {
	...
	var table = function table(element, options) {
		...
		var columnDateKey = attr("column-date-key");
		var dateFilterType = attr("date-filter-type");
		var defaultStartAddNum = dateFilterType == "month" ? 0 : -14;
		var dateStartAddNum = attr("date-start-add-num", defaultStartAddNum);
		var dateEndAddNum = attr("date-end-add-num", 0);
		var url = attr("url", "../Table/" + id + "/");
		var curd = attr("curd", "curd");
		var relate = attr("relate", "");
		var attachment = attr("attachment") != undefined;
		...
		if (!data("init")) {
			...
			data("setCurrentPageHtml", function () {
				...
				node("tbody").children("tr").children(".pre").children(".attachment").children(".contain").children(".switch").delegate("", "click", function () {
					...
					thisAttachmentIcon.children(".contain").children(".panel").children(".head").children(".addon-upload").upload({ "hideCallback": function hideCallback() {
							data("refreshAttachment")(attachmentTbody, columnKeyValueArr);
						} });
			...
			var getRequestTdData = function getRequestTdData(td, type) {
				var result = "";
				switch (type) {
					case "multi-select":
			...
			var getRequestTdHtml = function getRequestTdHtml(d, v) {
				var result = "";
				var defaultValue = "";
			...
			(function () {
				...
				//create wall
				node("request").children(".create").wall({ "html": function html() {
				...
				if (setting.related != "" && $("#" + setting.related).attr("relateCreate") != undefined) {
					$("#" + setting.related).children(".request").children(".relateCreate").wall({ "html": function html() {
							var createHtml = "<div class='head'>";
							createHtml += "<button class='add btn btn-info'><i class='fa fa-plus'></i></button>";
							createHtml += "<button class='minus btn btn-warning'><i class='fa fa-minus'></i></button>";
							createHtml += "<button class='remove btn btn-danger'><i class='fa fa-times'></i></button>";
				...
				node("request").children(".attachmentBatchDownload").delegate("", "click", function () {
					if (confirm("你确定要下载勾选的" + n + "行数据的所有附件吗？")) {
						...
						http.request(url + "AttachmentBatchDownload", requestData).then(function (result) {}, function (result) {
				...
				//column filter
				node("column-filter").select({ "data": data("thArr"), "selectCallback": function selectCallback() {
				...
				node("row-filter").wall({ "html": function html() {
